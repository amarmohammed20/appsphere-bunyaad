...
- [ ] **1. Streamline GitHub Actions** - the remaining items are CI
  ...
  - [ ] One `ci.yml`: path-filtered fan-out to parallel jobs, reconverging on a
        single `ci-ok` required check.
  - [ ] Shared composite action for setup (pnpm + Node + install + cache).
  - [ ] Cache `.next/cache`. The pnpm store is already cached by `setup-node`
        ...
        gitignoring it does not apply here.
  - [ ] One secret scanner per context. Bunyaad already runs a single TruffleHog
        version (v3.96.0) everywhere, so Sam's version-mismatch finding does not
        apply, but `pr-checks` and `security-gate` still both scan the same PR.
...
- [x] **4. Dependabot vs Renovate** - `renovate.json` is complete. Bunyaad uses
      its existing three-day release age rather than Sam's suggested 14 days;
      enabling the Renovate GitHub App remains a separate owner action.
...
## Current branch: `feat/security-pipeline`

- [x] Add zizmor as a blocking GitHub Actions security check.
- [x] Add ShellCheck for the repository's shell scripts and hooks.
- [x] Add actionlint for GitHub Actions workflows and embedded shell.
- [x] Add conservative Renovate configuration with no automerge. Bunyaad will
      use its existing three-day release age rather than Sam's suggested 14
      days.
- [x] Replace the Semgrep runtime `pip install` with the official Semgrep
      `1.172.0` container pinned by digest, preserving existing behaviour.
- [x] Add checkov. It flagged `notify-slack.yml` for having no permissions
      block; that workflow has since been deleted as unused.
- [x] Delete `notify-slack.yml` (nothing ever called it) and
      `security-scheduled.yml` (its schedules were already disabled, so it only
      ran on a manual click). Both are recoverable from git history if the
      scheduled scans or Slack alerting are wanted later.
- [x] Act on what zizmor found rather than filtering it out: stop persisting the
      checkout credential in the three jobs that do not need it, and pass the
      build-config outputs into `github-script` through `env` instead of
